...
from app.pipeline.ranker import rank_and_dedupe_sources, resolve_canonical_domain
...
class PipelineService:
    async def run(self, run_id: str, competitor_name: str) -> None:
        ...
        try:
            ...
            add_event(run_id, "crawling", f"Crawling {min(len(discovered), 16)} pages in parallel.", 25)
            crawled = await crawl_sources(discovered)
            if not crawled:
                raise RuntimeError("Unable to crawl sources")

            update_run(run_id, status="extracting")
            add_event(run_id, "extracting", "Extracting and ranking evidence snippets.", 45)
            ...
            add_event(run_id, "extracting", "Scoring competitive signals.", 58)
            signals = extract_competitive_signals(snippets, ranked)

            update_run(run_id, status="generating")
            add_event(run_id, "generating", "Generating battlecard with citation-grounded claims.", 70)
